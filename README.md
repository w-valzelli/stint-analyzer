...
The app detects the Garage 61 session sheet, discovers sectors, preserves source fields, identifies duplicate file bytes, and checks track consistency across imports. Scope review hides incomplete laps from analysis while keeping them available in the lap audit. The analysis views now include an overview, runtime leaderboard, sector benchmarks and progression, consistency metrics, driver detail, and a sortable and filterable lap audit. Every displayed analysis fact comes from the canonical report. Exports remain planned for Milestone 6.
